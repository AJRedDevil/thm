...
class PricingTestCase(TestCase):
    ...
    def setUp(self):
        ...
        # create complexity rate
        # for _complexity_rate in [0.0, 50.0, 100.0, 150.0, 200.0]:
           #  complexity_rate = ComplexityRate(
           #    complexity_rate = _complexity_rate
           #    )
           #  complexity_rate.save()

        # hourly_rate = { "1": 400.0, "2": 350.0, "5": 300.0, "12": 250.0, "18": 200.0 }
        # _hour_rate = HourRate()
        # _hour_rate.hour_rate = hourly_rate
        # _hour_rate.save()

    def test1_pricingUrl(self):
        # Login to the system
        login_post_data = dict(
            phone=self.staffphone,
            password=self.staffpassword1
        )
        response = self.client.post('/signup/', data=login_post_data)

        # response = self.client.get(reverse('logout'))
        # logging.warn(response.status_code)
        # # logging.warn(response.url)
        # response = self.client.post('/signin/', data=login_post_data)
        # # Open Pricing Page
        # # response = self.client.get('/pricing/')
        # logging.warn(response.status_code)
        # logging.warn(response.url)
        # self.assertNotEqual(response.status_code, 200)
        # Create pricing post data
        # pricing_post_data = dict(
        #   time_unit_selection='0',
        #   estimate_time='1',
        #   complexity='1',
        #   discount='0'
        #   )
        # response = self.client.post('/pricing/',data=pricing_post_data)
        # logging.warn(response.status_code)
        # logging.warn(response.url)
